"""
lane_detector.py — Détection double voie (ruban adhésif noir sur papier blanc A4)
===================================================================================
Filtres :
  1. Aire minimale
  2. Élongation  : minAreaRect ratio >= min_elongation
  3. Hauteur bbox : >= min_height_ratio * hauteur ROI
  4. Sélection   : centroïde le plus bas (plus proche robot)
  5. Lissage EMA : évite les sauts brusques entre frames (contour stable)
  6. Saut max    : si nouveau centroïde trop loin du précédent → ignoré
"""

import cv2
import numpy as np
from esibot_vision.config import (COLOR_CYAN, COLOR_YELLOW, COLOR_GREEN,
                                   COLOR_GRAY)

_MIN_ELONGATION   = 2.5
_MIN_HEIGHT_RATIO = 0.10
_EMA_ALPHA        = 0.35   # lissage : 0=figé, 1=pas de lissage
_MAX_JUMP_PX      = 60     # saut max accepté entre 2 frames (px)
...
class LaneDetector:
    ...
    def detect(self, frame: np.ndarray, annotated: np.ndarray | None = None):
        ...
        # OPEN (5,5) : supprime petit bruit
        # CLOSE (3,3) : referme micro-trous du ruban SANS relier objets distants
        ...
        # ── Détection brute dans chaque moitié ───────────────────────────
        raw_left,  left_inner  = self._find_raw_centroid(mask[:, :mid_x],  roi_h, side="left")
        raw_right, right_inner = self._find_raw_centroid(mask[:, mid_x:],  roi_h, side="right")

        # x_offset pour coordonnées image complète
        left_raw_cx   = (raw_left  + 0)     if raw_left  is not None else None
        right_raw_cx  = (raw_right + mid_x) if raw_right is not None else None
        # Bords intérieurs (pour filtre obstacles)
        ...
        if left_cx is not None and right_cx is not None:
            ...
            if annotated is not None:
                line_len = (h - roi_y) // 4
                cv2.line(annotated, (int(lane_mid), h - 1),
                         (int(lane_mid), h - 1 - line_len), COLOR_GREEN, 2)
    ...
    def _smooth(self, raw_cx: float | None, attr: str, w: int) -> float | None:
        """Applique EMA + filtre saut max sur un centroïde."""
        prev = getattr(self, attr)

        if raw_cx is None:
            # Pas de détection cette frame → on décroit doucement vers None
            if prev is not None:
                # Garder la dernière valeur connue (robot continue sur inertie)
                pass
            return prev
        ...
        if abs(raw_cx - prev) > _MAX_JUMP_PX:
            return prev  # ignorer ce saut → garder ancienne valeur
    ...
    # ─────────────────────────────────────────────────────────────────────
    def _find_raw_centroid(self, half_mask: np.ndarray, roi_h: int,
                           side: str = "left") -> tuple[float | None, float | None]:
        """
        Retourne (centroïde_x, bord_intérieur_x) du meilleur contour-ruban.
        bord_intérieur = bord droit du ruban gauche / bord gauche du ruban droit.
        """
        cnts, _ = cv2.findContours(half_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if not cnts:
            return None, None
        ...
        # Bord intérieur : côté droit pour ruban gauche, côté gauche pour ruban droit
        if side == "left":
            inner = float(x + bw)   # bord droit du ruban gauche
        else:
            inner = float(x)        # bord gauche du ruban droit
    ...
    def _draw_tape(self, half_mask, annotated, roi_y, x_offset, smoothed_cx):
        """Dessine le contour du ruban et le centroïde lissé."""
        cnts, _ = cv2.findContours(half_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if not cnts:
            return
        roi_h_local = annotated.shape[0] - roi_y if annotated is not None else 100
        candidates = [c for c in cnts
                      if cv2.contourArea(c) >= self.min_area
                      and self._is_tape(c, roi_h_local)]
        if not candidates:
            return
        ...
        # Point centroïde lissé
        if smoothed_cx is not None:
            M = cv2.moments(c)
            if M["m00"] != 0:
                cy = int(M["m01"] / M["m00"])
                cv2.circle(annotated, (int(smoothed_cx), roi_y + cy), 6, COLOR_YELLOW, -1)
