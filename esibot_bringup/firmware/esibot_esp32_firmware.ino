/*
  ...
  CAMERA (WiFi):
    ESP32-CAM serves MJPEG stream at  http://<STATIC_IP>/stream
    esibot_camera node connects to this URL directly over WiFi.
    Same subnet as Raspberry Pi (10.191.115.0/24).
...
// Static IP — same subnet as Raspberry Pi (10.191.115.99)
// Change STATIC_IP if 10.191.115.10 is already taken on your network.
...
#define WIFI_TIMEOUT_MS 10000    // abort WiFi connect after 10 s
...
static bool initCamera() {
  ...
  if (psramFound()) {
    cfg.frame_size   = FRAMESIZE_QVGA;  // 320×240 — matches camera_params.yaml
...
// ═══════════════════════════════════════════════════════════════════════════════
//  MJPEG STREAM HANDLER  (runs in esp_http_server task — non-blocking for loop)
// ═══════════════════════════════════════════════════════════════════════════════
...
static esp_err_t streamHandler(httpd_req_t* req) {
  ...
  res = httpd_resp_set_type(req, STREAM_CONTENT);
  if (res != ESP_OK) return res;

  // Disable Nagle — send each chunk immediately.
...
void setup() {
  // Disable brownout — WiFi radio can pull >300 mA briefly.
  ...
  delay(200);
  Serial.println("EsiBot ESP32-CAM firmware starting...");
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (cameraOk) startStreamServer();
  } else {
    Serial.println("WiFi: FAILED — stream unavailable, motor control still active");
  }
...
//  (HTTP stream runs in its own FreeRTOS task via esp_http_server)
// ═══════════════════════════════════════════════════════════════════════════════
