...
class EsibotDriver(Node):
    def __init__(self) -> None:
        ...
        if self.get_parameter("sim_mode").value:
            self.get_logger().info("sim_mode=true — skipping serial connection, using simulated odometry.")
        else:
    ...
    def _cmd_vel_callback(self, msg: Twist) -> None:
        ...
        self._last_cmd_angular = max(-MAX_ANGULAR_VEL, min(MAX_ANGULAR_VEL, msg.angular.z))
        self._last_cmd_time    = self.get_clock().now()

        if self._serial_conn is None:
            return

        v_right = self._last_cmd_linear + (self._last_cmd_angular * self._wheel_base / 2.0)
        v_left  = self._last_cmd_linear - (self._last_cmd_angular * self._wheel_base / 2.0)
